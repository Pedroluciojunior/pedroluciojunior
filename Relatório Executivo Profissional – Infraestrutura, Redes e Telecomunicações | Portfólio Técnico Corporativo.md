# RELATÓRIO EXECUTIVO PROFISSIONAL  

## 👨‍💼 Pedro Lucio Cardoso Matos Junior  

Especialista em Infraestrutura, Redes e Telecomunicações  
Brasília – DF – Brasil  

---

## 📌 SOBRE ESTE REPOSITÓRIO

Este repositório contém meu Relatório Executivo Profissional consolidado, documentando minha experiência técnica e estratégica em:

- Infraestrutura de TI
- Redes Corporativas
- Telecomunicações
- Gestão de Incidentes e SLA
- Governança baseada em ITIL
- Segurança da Informação (ISO/IEC 27001)

O documento foi estruturado em formato corporativo, adequado para apresentação a:

- Gestores de TI  
- Recrutadores técnicos  
- Empresas de tecnologia  
- Processos seletivos estratégicos  

---

## 🏢 EXPERIÊNCIA CONSOLIDADA

### 🔹 NAVA – Technology for Business  
Especialista em Telecomunicações (2024 – Atual)

- Gestão de reparos em circuitos WAN  
- Troubleshooting em NWB, Datacom e IP ADSL  
- Garantia de SLA contratual  
- Interface com operadoras e equipes técnicas  

---

### 🔹 Ilha Service Tecnologia e Serviços  
Analista de Suporte (2020 – 2024)

- Active Directory  
- Microsoft 365  
- Diagnóstico de redes corporativas  
- Gestão de incidentes via ITSM  

---

### 🔹 Montreal Informática  
Técnico de Campo / Helpdesk

- Implantação de estações corporativas  
- Suporte N1/N2  
- Configuração LAN  
- Atendimento baseado em ITIL  

---

### 🔹 Transdourada Transportes Ltda.  
Suporte Técnico

- Administração de rede local  
- Backup e continuidade operacional  
- Suporte a sistemas administrativos  

---

## 🛠 COMPETÊNCIAS TÉCNICAS

- Administração de Redes  
- Telecomunicações (WAN / Circuitos de Dados)  
- Segurança da Informação  
- ISO/IEC 27001  
- ITIL  
- Troubleshooting Avançado  
- Gestão de SLA  
- Active Directory  
- Microsoft 365  

---

## 📜 CERTIFICAÇÕES

- MD-100 – Windows 10  
- ITIL Foundation  
- ISO/IEC 27001  
- Scrum Foundation  
- CCNA – Introdução às Redes  

---

## 📄 DOCUMENTO COMPLETO

O relatório detalhado está disponível em:

- 📘 `relatorio-executivo-premium.pdf`
- 📘 `relatorio-executivo-premium.md`

---

## 🎯 OBJETIVO PROFISSIONAL

Atuar em posição estratégica na área de Infraestrutura, Redes ou Telecomunicações, contribuindo para:

- Alta disponibilidade
- Segurança da Informação
- Governança de TI
- Eficiência operacional

---

## 📬 CONTATO

LinkedIn: (Inserir link)  
E-mail: (Inserir e-mail profissional)

---

> Documento estruturado em padrão executivo corporativo.
